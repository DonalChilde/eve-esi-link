...
from eve_esi_link.esi_link import EsiLink, _make_esi_response_group
...
def test_context_manager_initializes_and_closes_dependencies(
    ...
    monkeypatch.setattr(
        "eve_esi_link.esi_link.SqliteCacheFactory",
        fake_cache_factory,
    )
    monkeypatch.setattr(
        "eve_esi_link.esi_link.AiolimiterRateLimiterFactory",
        fake_rate_limiter_factory,
    )
    monkeypatch.setattr(
        "eve_esi_link.esi_link.api_request.ApiRequester",
        fake_api_requester_constructor,
    )
    monkeypatch.setattr(
        "eve_esi_link.esi_link.SqliteAuthManager",
...
def test_make_requests_validates_builds_processes_and_groups(
    ...
    monkeypatch.setattr(
        "eve_esi_link.esi_link.build_runtime_esi_request",
...
def test_validate_request_logs_and_reraises(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "eve_esi_link.esi_link.validate_esi_request",
        lambda esi_request, schema: (_ for _ in ()).throw(error),
    )
    monkeypatch.setattr(
        "eve_esi_link.esi_link.logger.error",
